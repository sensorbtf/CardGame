...
public class HandVisual : MonoBehaviour
{
    ...
    public void PlayASpellFromHand(GameObject CardVisual)
    {
        ...
        Sequence s = DOTween.Sequence();
        s.Append(CardVisual.transform.DOMove(GlobalSettings.Instance.Players[owner].PArea.PDiscard.transform.position, 0.4f));
        s.Insert(0f, CardVisual.transform.DORotate(new Vector3(0f, -180f, 0f), 0.4f));
        s.OnComplete(() =>
        {
            Command.CommandExecutionComplete();
            Destroy(CardVisual);
    ...
    {
        Debug.Log("Discarding a card with index DiscardCard: " + index);
        GameObject cardToDiscard = CardsInHand[index];
        RemoveCardAtIndex(index);
        Sequence s = DOTween.Sequence();
        // move cards to the discardpile
        s.Append(cardToDiscard.transform.DOMove(GlobalSettings.Instance.Players[owner].PArea.PDiscard.transform.position, 0.4f));
        s.Insert(0f, cardToDiscard.transform.DORotate(new Vector3(0f, -180f, 0f), 0.4f));
        s.OnComplete(() =>
        {
    ...
    {
        Debug.Log("Discarding a card with index: " + index);
        GameObject cardToDiscard = CardsInHand[index].gameObject;
        RemoveCardAtIndex(index);
        Sequence s = DOTween.Sequence();
        // move cards to the deck
        s.Append(cardToDiscard.transform.DOMove(GlobalSettings.Instance.Players[owner].PArea.PDeck.transform.position, 1f));
        s.Insert(0f, cardToDiscard.transform.DORotate(new Vector3(0f, -179f, 0f), 1f));
        s.OnComplete(() =>
        {
            Destroy(cardToDiscard.gameObject);
            Command.CommandExecutionComplete();
